...
function MainFrame:BuildTabs()
	...
	self.tabs = {}
	local defs = {
		{ id = "session", label = "Session", scroll = f.Content.SessionScroll, panel = nil },
		{ id = "ninja", label = "Ninja List", scroll = f.Content.NinjaScroll, panel = nil },
		{ id = "settings", label = "Settings", scroll = nil, panel = f.Content.Settings },
	}
	local x = 0
...
function MainFrame:ShowTab(tabId)
	...
	for id, btn in pairs(self.tabs) do
		...
		btn.label:SetTextColor(active and 1 or 0.6, active and 0.85 or 0.6, active and 0.2 or 0.6)
	end
	self.currentTab = tabId
	if tabId == "session" then LG.SessionTab:Refresh()
	elseif tabId == "ninja" then LG.NinjaListTab:Refresh() end
end
...
function MainFrame:Toggle(show)
	...
	if show then
		f:Show()
		LG.Theme:FadeIn(f, 0.15)
		self:ShowTab(self.currentTab or "session")
	else
		LG.Theme:FadeOut(f, 0.15)
		f:Hide()
	end
end
